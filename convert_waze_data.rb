...
usage = %(convert_waze_data will convert Waze data collected with Mercator
projection to WGS84 projection used by the GPS system.

Script usage:
convert_waze_data.rb <input file> <output file> <index for coordinate column>
)
...
enc = 'BOM|UTF-16LE:UTF-8'
sep = "\t"

# Extract locations from input file and write them to a temporary file
line = 1
File.open(tmp_file, 'w') do |f|
  CSV.foreach mercator_file, col_sep: sep, encoding: enc do |row|
...
# Invoke the proj.4 command to convert from the Waze Mercator projection to
# WGS84 used by Leaflet, Google maps and others
...
CSV.open(wgs84_out_file, 'w', col_sep: sep) do |outfile|
  CSV.foreach mercator_file, col_sep: sep, encoding: enc do |row|
    unless row[col].nil? || !row[col].include?(',')
